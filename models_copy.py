...
def getAuthorsName(id):
    ...
    for n in narrators:
        n['_id'] = str(n['_id'])
        #n['narrator_ar'] = str(n['narrator_ar'])
    return narrators
...
def getHadithBooks(id,f_book,had):
    if(id !=None):
        query = {"collection":ObjectId(id)}
    else:
        query = {}
    book = list(db.HadithBook.find(query,
                                   {"_id":1,"booktitle_ar":1,"flag":1}))
    if(f_book is None):
        f_coll,f_book,f_chap,had = get_hadiths_ch()
    ids_list1 = [d['_id'] for d in book]
    ids_list2 = [d['bookId'] for d in had]
    missing_ids_in_list1 = [ObjectId(id) for id in ids_list1 if id not in ids_list2]
    f_book.extend(missing_ids_in_list1)
    for b in book:
        b['_id'] = str(b['_id'])
...
def getHadithChapters(id,ch_id,f_chap,had):
    bkid = id
    if(id !=""):
        query = {"book":ObjectId(id)}
    else:
    ...
    if f_chap == None:
        f_coll,f_book,f_chap,had = get_hadiths_ch()
    ids_list1 = [d['_id'] for d in book_chap]
    ids_list2 = [d['chapterId'] for d in had]
    missing_ids_in_list1 = [ObjectId(id) for id in ids_list1 if id not in ids_list2]
    f_chap.extend(missing_ids_in_list1)
...
def get_loaddata(c_id,b_id,ch_id):
    ...
    ids_list1 = [d['_id'] for d in collection]
    ids_list2 = [str(d['collectionId']) for d in had]
    missing_ids_in_list1 = [ObjectId(id) for id in ids_list1 if id not in ids_list2]
    f_coll.extend(missing_ids_in_list1)
    coll_lst = []
    b_lst = []
    ch_lst = []
    hadith = []
    ...
    for c in collection:
        ...
        if(first_coll_count==0):
            if (c_id != None):
                id = c_id
            book = getHadithBooks(id,f_book,had)
            book_lst = []
            first_book_count = 0
            for b in book:
                book_id = b["_id"]
                ...
                if(first_book_count == 0 and b['flag']==1):
                    if(b_id!=None):
                        book_id = b_id
                    chapter,hadiths = getHadithChapters(book_id,ch_id,f_chap,had)
                    for ch in chapter:
...
def insert_Comments(id,hadeethComment,sanadComment,hadiths,saveflag,hadithno):
   
    myquery = { "_id" : ObjectId(id) }
    newvalues = {"$set" :{"hadithComment": hadeethComment,
                            "chainComment": sanadComment,
                            "save_flag":saveflag,
                            "body_ar":hadiths,
                            "hadithno":hadithno}}
...
def insert_Book(data):
    ...
    st = x.inserted_id
    coll_count = getHadithBooks(data['collection'],None,None)
    newvalues = ""
    if(len(coll_count)<1):
        newvalues = { "$set": { "flag": 0 } }
    else:
